...
//		GUILayout.Space(6);
	...
	private void applySpeed()
	{
		foreach(GameObject go in Selection.gameObjects)
		{
			...
			{
                var mainMod = ps.main;
                mainMod.simulationSpeed = (100.0f/LTScalingValue);
			}
		}
	}
	...
	private void CopyModules(ParticleSystem source, ParticleSystem dest)
	{
		...
		if(b_modules[0])
		{
            ...
            var mainSource = source.main;
            var emissSource = source.emission;
            var mainDestMod = dest.main;
            var emissDestMod = dest.emission;

            mainDestMod.startDelay = mainSource.startDelay;
            mainDestMod.loop = mainSource.loop;
            mainDestMod.playOnAwake = mainSource.playOnAwake;
            mainDestMod.simulationSpeed = mainSource.simulationSpeed;
            emissDestMod.rateOverTime = emissSource.rateOverTime;
            mainDestMod.startSpeed = mainSource.startSpeed;
            mainDestMod.startSize = mainSource.startSize;
            mainDestMod.startColor = mainSource.startColor;
            mainDestMod.startRotation = mainSource.startRotation;
            mainDestMod.startLifetime = mainSource.startLifetime;
            mainDestMod.gravityModifier = mainSource.gravityModifier;
		}
	...
	private void ScaleParticleValues(ParticleSystem ps, GameObject parent)
	{
        //Particle System
        var mainMod = ps.main;
        mainMod.startSizeMultiplier = ScalingValue;
		mainMod.gravityModifierMultiplier = ScalingValue;
		if(mainMod.startSpeed.Evaluate(0) > 0.01f)
			mainMod.startSpeed = ScalingValue;
